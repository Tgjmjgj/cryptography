...
namespace MD55
{
    static class EMDE5
    {
        ...
        public static string Create(string msg)
        {
            ...
            {
                uint[] X = new uint[16];
                for (int ii = 0, j = 0; j < 64; ii++, j += 4)
                {
                    X[ii] = ((uint)msg_for_decode[64*i + j]) | (((uint)msg_for_decode[64*i + j + 1]) << 8) |
                           (((uint)msg_for_decode[64*i + j + 2]) << 16) | (((uint)msg_for_decode[64*i + j + 3]) << 24);
                }
                AA = A;
                BB = B;
                CC = C;
